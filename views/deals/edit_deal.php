...
$id = $_GET['id'];
$deal = getDealById($conn, $id);
$items = getDealItems($conn, $id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (updateDeal($conn, $id, $name, $price, $items_new)) {
        header("Location: index.php");
        exit;
    } else {
        $errorMessage = "❌ Error updating deal.";
    }
}
?>
        ...
        <form method="post">
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="name" placeholder="Enter Deal Name" value="<?= htmlspecialchars($deal['name']) ?>" required>
                </div>
                <div class="form-group">
                    <label>Price</label>
                    <input type="number" step="0.01" name="price" placeholder="Enter Deal Price" value="<?= htmlspecialchars($deal['price']) ?>" required>
                </div>
            </div>
            ...
            <div id="deal-items">
                <?php foreach ($items as $item): ?>
                    <div class="deal-item-row">
                        ...
                        <input type="number" name="quantity[]" value="<?= htmlspecialchars($item['quantity']) ?>" step="0.01" required>
            ...
            <div style="margin-top:20px;">
                <button type="submit" class="btn-submit">Update Deal</button>
            </div>
        </form>
